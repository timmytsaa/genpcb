...
- [docs/review-repair-import.md](docs/review-repair-import.md) — 審查/修改/匯入(檔案+圖片)整合路線：審查≈checker、repair 用擾動資料、圖片獨立 track
- [docs/results-grpo-v0.md](docs/results-grpo-v0.md) — GRPO v0 結果：迴圈驗證通過(100% 合法)、placement proxy 飽和、轉向 routing reward
- [docs/results-grpo-surrogate-v1.md](docs/results-grpo-surrogate-v1.md) — GRPO v1(surrogate reward)+真值 audit：infrastructure 完整、GRPO 有效但未贏基線(0.61 vs 0.76)、瓶頸=SFT/LLM 品質非 surrogate
...
src/genpcb/
  ...
  rewards/          Tier-1 placement reward（確定性，可跑）+ shaped/strict completion 橋
  kicad/            IR ↔ .kicad_pcb 橋（P0，round-trip 驗證；Freerouting 待 KiCad）
