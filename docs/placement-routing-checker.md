...
**實作進度（2026-06-11）**：Tier-1 **placement** 子集已實作且本機驗證——`genpcb.rewards.metrics`（courtyard 重疊/越界/HPWL/RUDY/decap，純幾何不需 KiCad）、`placement_reward` 聚合、`reward_from_completion`（policy 輸出 → reward 橋，malformed/incomplete → floor）、`grpo.make_reward_fn`。已驗證：reward 正確排序（baseline > collapsed/jittered/oob，24 板）、防「全疊一點」hack、8 類畸形輸入全 floor、完整性檢查防「少擺元件」hack。RUDY 因無界未校準暫不入 reward（w_rudy=0）。**未實作（需 KiCad）**：routing/DRC、dsl→.kicad_pcb 還原器、Freerouting。

| Phase | 內容 | 產出 |
|-------|------|------|
| 0（~2 週） | `pcb_metrics` 確定性檢查庫：KiCad 9 + pcbnew + kicad-cli 包裝、graph/raster 抽取器 | Tier 1 reward 可用（placement 子集✓，routing 待 KiCad） |
